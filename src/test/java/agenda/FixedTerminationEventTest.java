...
public class FixedTerminationEventTest {
    ...
    public void canCalculateNumberOfOccurrencesFromTerminationDate() {
        // Vérifier que la méthode calcule correctement le nombre d'occurrences
        assertEquals(9, fixedTermination.getNumberOfOccurrences());
    }
    ...
    public void occursInTerminationDay() {
        // Calcul de la date finale attendue
        LocalDate lastDay = LocalDate.from(fixedTermination.getStart().plusWeeks(fixedTermination.getNumberOfOccurrences() - 1));

        // Vérification de la présence dans le dernier jour attendu
        assertTrue(fixedTermination.isInDay(lastDay));
    }
